...
### Concept A (primary): <Sensor Activated Vacuum Rotating Lure Extermination Device>
...
**How it would be used:** <1.1. Using attractants, spotted lanternflies will either enter the trap or trigger a motion sensor that activates a low pressure vacuum to pull them inside. 2. A rotating blade guides them toward the center of the trap. 3. Continued rotation forces them downward through the center into the execution chamber. 4. A second rotating blade in the chamber kills the spotted lanternflies. >  
...
**End-of-semester proof-of-concept:** <On the mechanical side, we can design and build most of the trap, except for the vacuum system, which may be difficult given our budget and resources. We are also uncertain about the attractants. Although we can research sounds, scents, or flavors that draw SLF, we likely cannot conduct our own experiments on that aspect.>  

<Only include if you want the client to compare two directions. Keep short.>
## Key risks/unknowns [Serkan]

- Risk 1: Competitive Attraction. Natural grapevine phloem may outperform the artificial lure during ripening. Test: Conduct a 24-hour choice test between a high-sugar plant and the lure to determine the capture efficiency ratio.
Risk 2: SLF Evasion. The rotating mechanism might trigger a flight response. Test: Use video analysis to identify the optimal rotational speed that minimizes startle-induced jumping or flying.
Risk 3: Threshold Failure. Near-total capture is required as only two SLF can cause bin rejection. Test: Place the trap in a closed grapevine system for three weeks and perform a manual count to verify 100% capture.

## Questions for the client
1. Vineyard Spacing: What are the specific dimensions between rows? This determines if traps can be placed within the vineyard or must be restricted to the perimeter to avoid interfering with harvesters.
2. Manual Labor: Is on-site labor available to move traps during harvest? This determines whether the units need to be fixed-position or portable.
3. Certifications: Does the vineyard hold organic or food-grade certifications? This determines which chemical lures or artificial scents are permissible under your regulatory standards.
